...
def cuentaBombas(template_in, linepos, stringpos):
...
def detectaBombas(template_in):
    '''Se recorren las posiciones de la lista de strings,
     si es una bomba no se cambia y si no se buscan las bombas de alrededor con la funcion cuentaBombas 
    ...
    for i in range(len(template_in)):
        ...
        for j in range(len(template_in[i])):
            if template_in[i][j] == '*':
                ...
                lst_string.append(str(cuentaBombas(template_in, i, j)))
        template_out.append("".join(lst_string))
    return template_out

print()
for e in detectaBombas(template_minas):
    print(e)
